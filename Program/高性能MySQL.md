...
LIMIT 加上偏移量可用于分页，同时需要 ORDER BY，创建好对应的索引，效率会不错；  
但当偏移量很大时，需要查很多条记录，代价很高；简单的优化可使用索引覆盖扫描，只扫描索引，然后做一次关联操作再返回所需的列（即延迟关联）；  
有时可将 LIMIT 查询转换为范围扫描，比如新增一个递增排序列并创建索引，然后预先计算出边界值，再进行范围查询；  
或者记录下上次翻页的最后一个主键，并查询大于该主键的记录，这样无论翻页到多后，性能都不错；  
其它优化方法就是使用预先计算的汇总表，或者关联一个冗余表，只包含主键列和需要排序的列；  

### SQL_CALC_FOUND_ROWS

分页时，LIMIT 语句中加上 SQL_CALC_FOUND_ROWS 提示可获取满足条件的全部行数，可作为分页总数，但会导致扫描全部满足条件的行，浪费性能；  
可尝试每次读取比每一行数多一的行，比如每页 10 行，就读 11 行，若有第 11 行，则显示下一页按钮，不显示总页数；  
或者缓存较多数据，比如 1000 条，每次翻页从中取；  
有时可将 EXPLAIN 的结果中的 rows 列的值作为总数近似值

### UNION

UNION 查询总是创建并填充临时表，很多优化策略的不好用，需要将 WHERE, LIMIT, ORDER BY 等子句下推到 UNION 的各个子查询中，才能让优化器利用充分利用这些条件来优化；  
除非需要服务器消除重复行，否则要用 UNION ALL，若没有 ALL 关键字，临时表会被加上 DISTINCT 选项，导致对整个临时表数据做唯一性检查，代价很高；  

### 使用用户自定义变量

使用 SET 和 SELECT 定义，`:=` 进行赋值；  

不能使用自定义变量的场合：
* 使用自定义变量的查询，无法使用查询缓存
* 不能在使用常量或标识符的地方使用，比如表名、列名和 LIMIT 子句中
* 生命周期为一个连接，不能用于连接间的通信
* 若使用连接池或持久化连接，自定义变量可能被共用而产生影响（一般时代码 BUG 或连接池 BUG）
* 5.0 以前版本，大小写敏感
* 不能显示声明类型，确定类型的实际因版本而不同，最好在初始化时赋予类型相应的值，变量类型在赋值时发生改变，即动态类型
* 优化器可能将变量优化掉，导致与预想不符的行为
* 赋值的顺序和时间点不固定，依赖于优化器的决定
* 赋值符合 `:=` 优先级低，赋值表达式应使用明确括号
* 使用未定义变量不会产生语法错误！
* 可在赋值的同时使用变量，比如用在 SELECT 语句中
* WHERE 子句中表达式会比 SELECT 部分的表达式先执行，且读取一行记录就执行一次 WHERE 和 SELECT；但若含有 ORDER BY 子句，且使用文件排序，先对所有行执行 WHERE，再执行排序，最后 SELECT ? 关于顺序还有很多疑问；
...
分区表对用户来说是一个独立的逻辑表，但底层由多个物理子表组成；实现分区的代码是对一组底层表的句柄对象的封装，对分区表的请求，都由句柄对象转化成对存储引擎的接口调用；索引也是按分区的子表定义的，没有全局索引；  
建表时用 PARTITION BY 子句定义每个分区存放的数据，查询时优化器根据分区定义过滤掉没有所需数据的分区；  
一个主要目的是将数据按照一个较粗的粒度分在不同表中，将相关数据放在一起；另外，想一次批量删除整个分区的数据也更方便  

#### 适用场景：  

* 表很大，无法全部放入内存，或只有部分是热点数据，其它是历史数据
* 分区表数据更容易维护，删除大量数据可直接清除整个分区，可对一个独立分区进行优化、检查、修复等操作
* 数据可分布在不同物理设备上
* 可避免某些特殊瓶颈，如 InnoDB 的单个索引互斥访问，ext3 文件系统的 inode 锁竞争等
* 可备份和恢复独立分区

##### 限制：  

* 一个表最多 1024 个分区
* 在 5.1 中，分区表达式必须是整数（或返回整数）；5.5 中，某些场景可直接使用列进行分区
* 若分区字段有主见或唯一索引的列，则所有主键列和唯一索引列必须包含进来
* 无法使用外键约束

### 分区表原理

分区表由多个底层表实现，底层表也是由句柄对象（Handler object）表示的，可直接访问各个分区；存储引擎管理分区表的底层表和普通表一样（底层表必须使用相同存储引擎），它无须知道这是一个普通表还是分区底层表；分区表的索引只是在各个底层表上各自加上一个完全相同的索引；

#### 分区表操作逻辑

* **SELECT**：查询时，先打开并锁住所有底层表，优化器先判断是否可过滤部分分区，然后在调用对应的存储引擎接口访问分区数据
* **INSERT**：写入时，先打开并锁住所有底层表，确定哪个分区接收这条数据后将其写入
* **DELETE**：删除时，先打开并锁住所有底层表，确定数据对应分区后在该底层表将其删除
* **UPDATE**：更新时，先打开并锁住所有底层表，确定数据对应分区后取出并更新，再判断更新后数据应放在那个分区，进行写入，将原数据在原底层表进行删除

若 WHERE 条件恰好与分区表达式匹配，则可以过滤掉不符合的分区，而 INSERT 语句本身就只命中一个分区；  
每个操作都会**先打开并锁住所有底层表**，但并不是分区表处理过程中都锁住全表，若存储引擎实现了行级锁（如 InnoDB），则在分区层释放对应表锁

### 分区表类型

支持 RANGE, LIST, HASH, KEY 分区类型，5.5 中还有 RANGE COLUMNS 类型；  
最常用是根据数据范围分区（RANGE），分区表达式为**列或包含列的表达式**，可以使用函数，但表达式返回值必须是整数，且不能是常数

其它分区技术：
* 根据键值分区，减少 InnoDB 的互斥量竞争
* 使用数学模函数分区，将数据轮询放入不同分区，如对日期做模 7 运算
* 若表有自增主键 id，希望根据时间将最近的热点数据集中存放，则必须将时间戳包含在主键中，但这和主键本身的意义相矛盾；可使用 HASH(id DIV 1000000) 来分区，将 100 万数据建立一个分区，可避免使用时间范围分区时，若超过一定阈值，必须新增分区

### 如何使用分区表

若数据量巨大，全表扫描肯定不行，索引在空间和维护上的消耗也会很大，而且会有大量碎片；B-Tree 索引无法起作用，除非是索引覆盖扫描，否则服务器需要根据索引扫描结果回表查询，产生大量随机 I/O，索引维护代价也很高；  
分区相当于建立更粗粒度但消耗更少的检索方式，依靠分区表达式定位到一片分区中，然后可对其做顺序扫描，或建索引，或将数据缓存到内存等；  

为保证大数据量的可扩展性，一般有两个策略：
* **全量扫描数据，不用索引**：用简单方式分区，不建索引，用 WHERE 条件将所需数据限制在少数分区中，则效率很高
* **索引数据，并分离热点**：若只有热点数据经常访问，可将热点数据单独放在一个分区中，可建立索引，也可使用缓存

### 出问题

以上分区策略基于两个假设：查询都能过滤掉很多额外分区，分区代价小；  
但在某些场景这两个假设可能出现问题

#### NULL 值会使分区过滤无效

第一个分区是特殊分区，若分区表达式的值为 NULL，则将记录存放到第一个分区；若表达式为 YEAR(date), TO_DAYS(date) 等日期函数，若 date 为非法日期，则值也为 NULL，在类似 `WHERE date BETWEEN '2012-01-01' AND '2012-01-31'` 条件的查询中，因为这个条件的 date 可能包含非法值，所以 MySQL 将会额外检查第一个分区，浪费性能，为避免此情况，可创建一个空的第一分区 ?  
在 5.5 中则不需要这样优化，因为 `PARTITION BY RANGE COLUMNS(date)` 可直接使用列本身而不是基于列的函数来分区

#### 分区列与索引列不匹配

若分区列与索引列不匹配，比如在列 b 上分区，在列 a 上定义索引，由于每个分区都有独立索引，所以扫描列 a 上的索引就需要扫描每个分区内对应的索引，除非查询中还同时包含可以过滤分区的条件；所以分区列和索引列最好匹配

#### 选择分区的成本

不同类型的分区实现方式不同，性能也不同，尤其是范围分区，要过滤分区，服务器需要扫描所有的分区定义的列表，随着分区数的增长，成本会越来越高；  
可限制分区数量，根据经验，100 个左右分区一般没有问题
其它类型分区，如 KEY、HASH 分区，没有这样的问题

#### 打开并锁住所有底层表的成本

在分区过滤前发生，无法通过分区过滤降低此开销，且与分区类型无关；对于本身操作很快的查询，如根据主键查询单行，会有很明显的影响；  
可用批量操作的方式降低多次单个操作的开销（批量插入/删除、LOAD DATA INFILE 等），还有限制分区数量

#### 维护分区成本

新增和删除分区很快，但有些操作如**重组分区**或类似 **ALTER 语句**，需要复制数据，成本较高（重组分区的原理和 ALTER 类似，先创建一个临时的分区，然后将数据复制到其中，再删除原分区）

### 查询优化

查询分区表时，最后在 WHERE 条件带上分区列，优化器才能过滤不需要的分区，但优化器只有在使用列本身进行比较时才能过滤分区，而不能根据表达式的值去过滤，即使这个表达式就是分区表达式也不行；  
EXPLAIN 的 partitions 列显示过滤后的分区；  
关联查询时，若分区表为第二张表，且关联条件是分区列，则只会在对应分区内匹配行（这是运行时过滤的，不是查询优化阶段的，EXPLAIN 无法显示）

## 7.2 视图

视图有两种实现办法：
* **合并算法 (MERGE)**：查询时，将视图的定义 SQL 包含到查询的 SQL 中，再一起进行优化
* **临时表算法 (TEMPTABLE)**：查询时，将视图结果存放在临时表中，再在这个临时表上进行查询，优化器很难优化

若视图包含 **GROUP BY、DISTINCT、聚合函数、UNION、子查询** 等，无法在原表记录和视图记录建立一一映射的场景中，MySQL 将使用临时表算法实现视图，EXPLAIN 一条针对该视图的查询，select_type 列显示 DERIVED;  
也可强制使用临时表算法：`CREATE ALGORITHM=TEMPTABLE VIEW view1 AS ...;`

### 可更新视图

此处更新指**修改、删除、新增**；  
使用临时表算法实现的视图无法更新，且更新的列必须来自同一个表；  
创建视图时，在最后加上 `WITH CHECK OPTION` 可限制通过视图更新的行，必须符合视图本身的 WHERE 条件，若不符合，则报错；  
MySQL 不支持在视图上建触发器；  
...
可通过视图实现基于列的权限控制，而不需要在系统创建列权限，没有额外开销，如：
```SQL
--从私有的表中公开部分列
CREATE VIEW public.employee AS
    SELECT name FROM private.employee;
GRANT SELECT ON public.* TO public_user;    --设置权限
```

伪临时视图：可在一个连接中建立一个视图（注意名字的唯一性），在该视图上进行查询，连接结束时删除该视图，使用起来和子查询相同，但 MySQL 处理视图和处理子查询的代码路径不同，性能也不同；  

### 视图的限制

MySQL 不支持物化视图（将视图结果数据存放到一个表中，定期从原始表更新数据过去），不支持在视图中建索引，只能构建缓存表或汇总表来模拟

## 7.4 在 MySQL 内部存储代码

触发器、存储过程、函数、定时任务（事件）  
存储过程和函数可以有接收参数和返回值，触发器和事件没有
...
* 简化代码的维护和版本更新
* 安全性，如更细粒度的权限控制
* 服务器端可缓存存储过程的执行计划，降低反复调用的消耗
* 备份维护都在服务器端，没有外部依赖，比较方便
* 分工明确，可由数据库专家开发存储代码，与应用开发分开

存储代码缺点：
* MySQL 本身没有好用的开发和调试工具
* 代码效率较差，可使用的函数有限，很难实现复杂逻辑
* 增加代码部署的工作量
* 部署于服务器内，可能有安全隐患，如加密功能，若数据库被攻破则数据和加密方式都会泄露
* 给服务器带来额外压力，而数据库服务器扩展性相对应用服务器较差
* MySQL 没有选项可控制存储程序的资源消耗，出错可能拖垮服务器
* MySQL 存储代码的功能很弱，有诸多限制
* 调试困难
* 和基于语句的二进制日志复制合作得很不好
...
* 优化器无法评估存储函数的执行成本
* 每个连接有独立的存储过程的执行计划缓存，无法共用
* 存储程序和复制是一组诡异组合，最好不要复制对存储过程的调用，直接复制有存储过程改变的数据更好

### 触发器

MySQL 触发器限制：
* 每个表的每个事件，最多只能定义一个触发器
* 只支持“基于行的触发”，若变更的数据集很大，则效率很低

触发器限制：
* 掩盖了服务器背后的工作，使一条简单的语句，可能改变多个表的数据
* 问题很难排查
* 可能导致死锁和锁等待，若触发器失败，原 SQL 语句也会失败
* 触发器不一定能保证更新的原子性，比如在 MyISAM 中，触发器出错并不能回滚之前成功的更改；InnoDB 中则是原子性的，但要小心 MVCC，可能获得错误结果，除非用 SELECT FOR UPDATE

### 事件

事件在一个独立事件调度线程中被初始化（与处理连接的线程无关），可在日志中看到命令的的执行日志，在表 INFORMATION_SCHEMA.EVENTS 看到各个事件状态；  
事件如果执行时间很长，则可能在未完成时，下一时间点的同一事件就开始了，MySQL 不会防止这种并发，需要自己写防并发代码

## 7.5 游标

MySQL 只在服务器端提供只读、单向的游标，只能在存储过程或更底层的客户端 API 中使用；  
游标指向存储在临时表中的数据而不是实际查询的数据，因此，虽然游标是逐行读取数据，但在打开游标时，仍需完整执行查询，并将查询到的所有数据放入临时表中，有性能问题（MySQL 的锅），注意用 LIMIT 限制查询数量；  
临时内存表不支持 BLOB 和 TEXT 类型，若游标返回这类结果，MySQL 必须创建临时磁盘表来存放，性能可能很糟；或者当临时表大于 tmp_table_size 时，也会创建临时磁盘表

## 7.6 绑定变量

客户端向服务器发送一个 SQL 语句的原型，使用 '?' 标记可以接收参数的位置，服务器端收到后，解析并存储该语句的部分执行计划，返回一个 SQL 语句处理句柄，以后每次执行这类查询，客户端都指定使用该句柄；  
绑定变量相对安全，无须在应用程序中处理转义，更简单，也减少 SQL 注入和攻击的风险；  

使用绑定变量可以更高效地执行大量重复语句，原因：
* 服务器只需解析一次 SQL 语句
* 某些优化器工作只需执行一次，会缓存一部分执行计划
* 以二进制方式发送参数和句柄，比每次发送 ASCII 码文本效率更高，占用字节少，参数和句柄也比整条语句更小；BLOB 和 TEXT 等大型字段，绑定变量的形式可分块传输，无须一次性传输；二进制协议在客户端也能节省内存，减少网络开销，节省将数据从存储原始格式转换为文本格式的开销
* MySQL 存储参数时，直接放入缓存，不需要在内存中多次复制

### 绑定变量的优化

* 准备阶段，服务器解析 SQL 语句，移除不可能条件，并重写子查询
* 第一次执行时，若可能，服务器先简化嵌套循环的关联，并将外关联转换为内关联
* 每次执行时，服务器做如下事情（有些优化只需做一次，但实际上还是会执行所有操作）：
  * 过滤分区
  * 尽可能移除 COUNT(), MIN(), MAX()
  * 移除常数表达式
  * 检测常量表
...
不使用二进制传输协议也可直接以 SQL 的方式使用绑定变量：
```SQL
SET @sql := 'SELECT * FROM actor WHERE name = ?';
PREPARE stmt_fetch_actor FROM @sql;
SET @actor_name := 'jojo';
EXECUTE stmt_fetch_actor USING @actor_name;
```

最主要用于存储过程中，可灵活地拼接字符串等参数构建 SQL 语句

### 绑定变量的限制

* 绑定变量是会话级别的，连接之间不能共用绑定变量句柄，一旦连接断开，原来的句柄也不能使用
* 5.1 前，绑定变量的 SQL 不能用查询缓存
* 如果只执行一次，则更浪费性能，多了一次额外的准备阶段的消耗，还需要一次额外的网络开销（使用完成后，要释放相关资源）
* 不能在存储函数中使用绑定变量
* 若总是忘记释放绑定变量的资源，服务器容易发生资源泄露；绑定变量 SQL 总数限制是全局的，某个地方的错误可能影响其他所有线程
* 有些操作，如 BEGIN，无法在绑定变量中完成

## 7.7 用户自定义函数

用户自定义函数（UDF），可用支持 C 语言调用约定的任何编程语言实现，必须事先编译好并动态链接到服务器上；  
速度很快，可访问大量操作系统功能，使用大量库函数；  
错误可能让服务器之间崩溃，甚至扰乱服务器内存或数据，具有 C 语言所有潜在风险；  
无法读写数据表，至少，无法在调用 UDF 的线程中使用当前事务处理的上下文来读取数据表；  
必须确保 UDF 是线程安全的，因为 MySQL 是纯粹的多线程环境

## 7.9 字符集和校对

### MySQL 如何使用字符集

每种字符集都可能有多种校对规则，且都有一个默认校对规则，每个校对规则都针对某个特定字符集，和其它字符集无关；  
控制字符集的选项分为两类：创建对象时的默认值，在服务器和客户端通信时的设置；  

#### 创建对象时的默认值

MySQL 的服务器、数据库、表都有自己默认的字符集和校对规则，是逐层继承的默认设置，最底层的默认设置将影响你创建的对象；  

* 创建数据库时，根据服务器上的 character_set_server 设置来设定改数据库的默认字符集
* 创建表时，根据数据库来设置这个表的默认字符集
* 创建列时，则根据表来设置

#### 服务器和客户端通信时的设置

服务器和客户端通信时可能使用不同字符集，服务器端将进行必要的翻译转换工作：  
* 服务器总是将设客户端按 character_set_client 设置的字符集来传输数据和 SQL 语句
* 当服务器收到 SQL 语句时，先将其转换成字符集 character_set_connection，还使用这个设置来决定如何将数据转换成字符串
* 当返回数据或错误信息时，将其转换成 character_set_result

可使用 SET NAMES 或 SET CHARACTER SET 来改变设置

#### 比较字符串大小

若比较两个字符串的字符集不同，先将其转成同一个字符集，若两个字符集不兼容则报错，需要通过 CONVERT() 显式转换；  
MySQL 为每个字符串设置了“可转换性”，决定了值的字符集的优先级；  
可使用前缀（下划线开始）和 COLLATE 子句来指定字符串的字符集或校对字符集，如：
`SELECT _utf8 'Hello World' COLLATE utf8_bin;`

#### 特殊情况

* 诡异的 character_set_datatbase 设置：其默认值和默认数据库的设置相同，当改变默认数据库时，变量也会变
* LOAD DATA INFILE：当使用 LOAD DATA INFILE 时，总是将文件中的字符按字符集 character_set_database 来解析，5.0 后，可使用子句 CHARACTER SET 来设定字符集；但指定字符集的最后方法是先用 USE 指定数据库，再执行 SET NAMES 来设定字符集，最后加载数据；MySQL 在加载数据时，总是以同样的字符集处理所有数据，不管表中的列是否有不同字符集设定
* SELECT INTO OUTFILE：不做任何转码，除非使用 CONVERT() 对所有列手动转码
* 嵌入式转义序列：根据 character_set_client 的设置解析转义序列

#### 对查询的影响

某些字符集和校对规则可能需要更多 CPU 操作，消耗更多内存和存储空间，甚至影响索引的正常使用；  
不同字符集和校对规则间的转换可能带来额外开销，比如：
* 排序查询只有在要求的字符集与服务器数据的字符集相同是才能用索引排序，若指定别的校对规则，则需要用文件排序
