...
在 LINQ to Objects 的实现中，先返回左边序列第一个元素与右边序列的所有成对数据，接着返回左边序列第二个元素与右边序列的所有成对数据，以此类推。右边序列被缓冲处理，左边序列仍是流处理，所以应该尽可能将**小的序列放在右边**。当访问第一个数据对时才会开始执行，读取整个右边序列，来建立一个从键到生成这些键的值的映射，之后就不需要再次读取右边的序列了。
