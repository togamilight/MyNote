...
分组将源序列按 Key 来进行分组，生成 `IGroup<TKey, TElement>` 的对象，这是一个扩展了 `IEnumerable<TElement>` 的序列，含有一个 `Key` 属性。Key 可以是**单个属性**或**多个属性组成的匿名类型**。  
...
作为方法参数时，静态类型采用单一分发，执行时所调用的方法取决于覆盖（override）后的方法目标的实际类型，重载是在编译时确定的；而动态类型采用多重分发，根据执行时实参的类型，找出最合适的方法实现  
...
### EF的事务与MSDTC
* 在低于 6.0 的 EF 中不能指定 SaveChanges 时使用的事务
...
SQL Server 中的 `Rank()` 函数用来实现名次可并列的排名，在 EF 中没有对应的方法，可以用以下方式实现：
```CSharp
var ents = dbContext.Enters.Where(...);
var result = ents.OrderBy(x => x.score).Skip(1).Take(10).Select(
