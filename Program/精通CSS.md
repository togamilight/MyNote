...
对于结构相同但内容文本不同而导致长度不同的几列，可以为其设置**很大**的**底内边距**和绝对值**稍小**的**负底外边距**，再将布局容器设 `overflow: hidden`，布局容器将会根据文本最长的一列来切割各列高度，形成等高的列。如果需要圆角效果，需要为各列添加一个 `<div>` 并以**绝对定位** 定位到布局容器底部，然后应用底部圆角的背景图像
...
* 计算BFC的高度时，浮动元素也参与计算。（**可以用来清除子元素浮动**）
